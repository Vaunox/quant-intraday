...
| 2026-06-27 | P3X.1 Survivorship-free panel + real gate run (liquidity universe) | ☑ done | `feat/p3x-cross-sectional-factors` | Operator chose the **liquidity-proxy** path (no free PIT membership exists). Built free from NSE: bhavcopy ingester (P3X.1b, `data/ingest/bhavcopy.py` — both layouts, ISIN rename-keying, incl. delisted), liquidity universe (P3X.1c), panel builder (P3X.1d, `scripts/build_nifty500_panel.py`), split/bonus CA adjustment (P3X.1e, `corporate_actions.py` + fetch script), `--validated` wiring (P3X.1f). **REAL VERDICT (2016-2024, 2221d×2695 names → liq top-500 → 1065 ever-eligible, 95 monthly rebalances, CNC 0.22%): active IR −0.815, all paths negative → KILL.** Composite rank-IC **+0.051** (positive — signal real, not a bug; momentum/low-vol/residual carry it, Amihud doesn't); too weak to beat the EW benchmark after cost. EW benchmark is harder than the spec'd cap-weighted TRI (caveat). See `docs/iteration_log/p3x_cycle1_finding.md`; MLflow run `af66c366`. |
...
| 2026-06-27 | P3X.3 Cross-sectional forward-return labeling | ☑ done | `feat/p3x-cross-sectional-factors` | `research/labeling/cross_sectional.py`: monthly forward 21-session total return (regression target) + cross-sectional decile rank within the PIT universe (classification target); `label_times` (t0=rebalance → t1=t0+horizon) for purge/embargo; `month_end_rebalance_dates` helper. Strictly forward (no same-day leakage — verified: label uses only entry+exit close), ranks within universe only, tail without forward window dropped. 8 unit tests; mypy/ruff/black clean. |
| 2026-06-27 | P3X.4 Signal combination | ☑ done | `feat/p3x-cross-sectional-factors` | `research/factors/combine.py`: `sector_neutral_composite` + `SignalCombiner` interface (`EqualWeightComposite` zero-param baseline, `WeightedComposite` trial-charged). Sector-neutral z-score per factor then weighted nanmean; NaN factors skipped not zeroed. ML ensemble combiner deferred to P3X.5 eval (trained per fold, trial-charged). 6 unit tests; gates clean. |
| 2026-06-27 | P3X.5 Validation harness adaptation | ☑ done | `feat/p3x-cross-sectional-factors` | `research/validation/active.py`: `active_returns`, `active_information_ratio` (annualised, ×√12 monthly), `evaluate_active_under_cpcv` (reuses CombinatorialPurgedCV over rebalance blocks → per-obs active path-Sharpe distribution), `ActiveEvaluation` (annualised IR, DSR at honest N, **de-dup t-stat = path-concat/√φ** per FINDINGS §3). 4 unit tests; gates clean. |
| 2026-06-27 | P3X.6 CNC cost model + monthly backtester | ☑ done | `feat/p3x-cross-sectional-factors` | `research/validation/monthly_backtest.py`: `CncCostModel` (itemised delivery — **STT 0.10% both legs** + stamp + exchange/SEBI/GST → round-trip ≈ 0.0022, verified); `monthly_backtest` (next-period hold, no square-off, relative no-trade band applied before costing, drift ignored); `reconcile_net_returns` (vectorised vs explicit-loop, agree < 1e-12). 5 unit tests; gates clean. |
| 2026-06-27 | P3X.7 Portfolio construction + sizing (**built Capital Layer**) | ☑ done | `feat/p3x-cross-sectional-factors` | `capital/portfolio/construction.py`: `select_top_quantile`, `inverse_vol_weights`, `hrp_weights` (López de Prado — quasi-diag via single-linkage + recursive bisection; robust to singular cov, verified), `apply_caps` (name ≤5% + sector ≤25%, excess→cash), `construct_book` (select→weight→cap). `capital/sizing/sizing.py`: `vol_target_scalar` (quarter-Kelly, capped). The Capital Layer was empty scaffolding — now built for the factor product. 8 unit tests; gates clean. |
| 2026-06-27 | P3X.8 Pre-committed gate run — **code-complete (synthetic integration)** | ◑ built; real run data-gated | `feat/p3x-cross-sectional-factors` | `research/factors/gate.py`: seven-point benchmark-relative kill-gate as a pure threshold-checker (`GateMetrics` → `evaluate_factor_gate` → `FactorGateVerdict`; an uncomputed metric never passes). `scripts/run_factor_gate.py`: full end-to-end assembly (factors→composite→labels→book+caps→CNC backtest→active-IR CPCV+DSR→§4 gate→MLflow). **Synthetic GBM integration run proves the pipeline assembles and the gate fires**: 53 rebalances, 60 names, active IR −1.591, DSR 0.000, dedup t −3.37, **sector cap 0.20 ≤ 0.25 PASS**, overall **VERDICT: KILL** (correct — noise minus CNC cost). 4 gate unit tests; gates clean. **`--validated` refuses to run (no number fabricated) until P3X.1 survivorship-correct Nifty-500 data exists.** |
