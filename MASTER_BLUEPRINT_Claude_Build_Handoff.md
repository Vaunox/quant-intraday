...
| | P2A.1 | ☐ todo | | | Kite Connect subscription + developer app → `api_key`/`api_secret`, both recorded only via the secrets interface (`QUANT_SECRET_KITE_*`), never git; operator paid signup. Runbook `docs/operator_runbooks/P2A.1_kite_signup.md`. |
